...
import DriftAudit from "../../components/widgets/DriftAudit.astro";
...
motion or style pressure. LoRA and DreamBooth work the opposite way, and
they are not two names for one method. Both want a training pass on a
few images of the subject before the first shot rather than a reference
handed over at generation time, and both hold a subject harder than any
single reference image does. Where they part is what they train.
DreamBooth trains the model itself — its method is to "fine-tune all
layers of the model" — so what comes out is a whole checkpoint, at a
whole checkpoint's training cost and disk footprint. LoRA leaves the
pretrained model where it is: it "freezes the pre-trained model weights
and injects trainable rank decomposition matrices into each layer", so
what comes out is a small file that loads on top of a checkpoint you
already have.

Source: DreamBooth: Fine Tuning Text-to-Image Diffusion Models for Subject-Driven Generation — [https://arxiv.org/abs/2208.12242](https://arxiv.org/abs/2208.12242)\
Source: LoRA: Low-Rank Adaptation of Large Language Models — [https://arxiv.org/abs/2106.09685](https://arxiv.org/abs/2106.09685)
...
face crop as the only reference. The next tier adds one full-length
reference alongside the face, which is what fixes the coat length and the
boots across the cut. The next tier doubles that set with the same
character dry, half-length and full-length, so both costume states the
scene passes through have a reference behind them. The top tier keeps the
